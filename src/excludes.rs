...
impl Excludes {
    pub fn parse_excludes(&self, excludes: Option<Vec<&str>>) -> Result<Option<GlobSet>> {
        match excludes {
            Some(excludes) => {
...
mod tests {
    ...
    pub fn simple_parse() {
        ...
        let excludes = Excludes::default().parse_excludes(Some(vec)).unwrap();
    ...
    pub fn nothing_parse() {
        let excludes = Excludes::default().parse_excludes(None).unwrap();
