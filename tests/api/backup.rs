...
pub fn simple_backup_with_excludes() -> Result<()> {
    ...
    let validate_stats = af.validate(&ValidateOptions::default(), None).unwrap();
...
fn check_backup(af: &ScratchArchive) {
    ...
    assert_eq!(af.unreferenced_blocks(None).unwrap().count(), 0);
}
...
pub fn empty_file_uses_zero_blocks() {
    ...
    restore(&af, dest.path(), &RestoreOptions::default(), None).expect("restore");
